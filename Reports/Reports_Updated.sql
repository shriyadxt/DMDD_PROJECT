...
select * from SHRIYA_PROJECT.case_details;
...
select * from SHRIYA_PROJECT.customer;
...
SELECT EMPLOYEE_ID,SUM(RES) as TOT_RES,SUM(PEN) as TOT_PEN, ROUND((SUM(RES)/(SUM(PEN)+SUM(RES))) * 100) AS RES_PERCENTAGE FROM A GROUP BY EMPLOYEE_ID
